...
// 작성 기능동작 컨트롤러
	...
	public String reservatonContents(@RequestParam("bidx") int bidx, Model model) {
	...
	public String reviewWriteAction(BoardVo bv, @RequestParam("attachfile") MultipartFile attachfile,																		// filename)
		...
		if (!file.getOriginalFilename().equals("")) { // ! >> 존재한다면(.equals("")없는게 아니라면)
			...
			uploadedFileName = UploadFileUtiles.uploadFile(uploadPath, file.getOriginalFilename(), file.getBytes());// 업로드
...
// =========================================================
	...
	public String askWriteAction(BoardVo bv, @RequestParam("attachfile") MultipartFile attachfile, HttpSession session,
			HttpServletRequest request, RedirectAttributes rttr) throws IOException, Exception {
		MultipartFile file = attachfile;// 새로운 변수에 담아서 이름을 변경해줌 ++ attachfile로 변경해서 vo에 있는 파일네임과 충돌방지
		String uploadedFileName = "";
	...
	public String askContents(@RequestParam("bidx") int bidx, Model model) {
	...
	public String levelupContents(@RequestParam("bidx") int bidx, Model model) {
	...
	@RequestMapping(value = "askModify.aws")
	public String askModify(@RequestParam("bidx") int bidx, Model model) {

		BoardVo bv = boardService.boardSelectOne(bidx);
		model.addAttribute("bv", bv);

		String path = "WEB-INF/board/askModify";
		return path;
	}
	@RequestMapping(value = "askModifyAction.aws")
	public String askModifyAction(BoardVo bv, // 제목 내용 작성자 비밀번호 bv에 담아오기
		...
		if (!file.getOriginalFilename().equals("")) { // 원본파일 이름이 있으면
			uploadedFileName = UploadFileUtiles.uploadFile(uploadPath, file.getOriginalFilename(), file.getBytes()); // uploadedFileName에
																														// 파일
																														// 정보들
																														// 담기
		}
		...
		value = boardService.askUpdate(bv); // 서비스에서 구현한 업데이트 메서드 안에 세팅한 파일이름, 회원번호, 아이피 정보 담아서 사용하기
		System.out.println("value  : " + value);
		String path = "";
		if (value == 1) { // 성공하면
			path = "redirect:/board/askContents.aws?bidx=" + bv.getBidx(); // 글번호 달고 글 내용보여주기

		} else {
			rttr.addFlashAttribute("msg", "실패");
			path = "redirect:/board/askModify.aws?bidx=" + bv.getBidx(); // 실패하면 글번호 가지고 다시 수정하기 페이지로 보내기
	...
	public String reviewModifyAction(BoardVo bv, // 제목 내용 작성자 비밀번호 bv에 담아오기
			@RequestParam("attachfile") MultipartFile attachfile, // 첨부파일은 멀티파트로 담아오기
			HttpServletRequest request, // 세션 요청하기 위한 객체 생성 > 회원번호 담을 것
			RedirectAttributes rttr) // 리다이렉트 속성값 담기 위한 객체 생성
			throws Exception {
	...
	// ==================== 삭제기능=====================
	@RequestMapping(value = "reviewDelete.aws") // 여기는 메서드 없음 리뷰삭제
	public String reviewDelete(@RequestParam("bidx") int bidx, Model model) {
		model.addAttribute("bidx", bidx);
		String path = "WEB-INF/board/reviewDelete";
		return path;
	}

	@RequestMapping(value = "askDelete.aws") // 여기는 메서드 없음 문의 삭제
	public String askDelete(@RequestParam("bidx") int bidx, Model model) {
		model.addAttribute("bidx", bidx);
		String path = "WEB-INF/board/askDelete";
		return path;
	}

	@RequestMapping(value = "levelupDelete.aws") // 여기는 메서드 없음 문의 삭제
	public String levelupDelete(@RequestParam("bidx") int bidx, Model model) {
		model.addAttribute("bidx", bidx);
		String path = "WEB-INF/board/levelupDelete";
		return path;
	}

	@RequestMapping(value = "reservationDelete.aws") // 여기는 메서드 없음 문의 삭제
	public String reservationDelete(@RequestParam("bidx") int bidx, Model model) {
		model.addAttribute("bidx", bidx);
		String path = "WEB-INF/board/reservationDelete";
		return path;
	}

	@RequestMapping(value = "reviewDeleteAction.aws", method = RequestMethod.POST) // 리뷰 삭제기능
	public String reviewDeleteAction(@RequestParam("bidx") int bidx, @RequestParam("pwd") String pwd,
		...
		if (value == 1) {
			...
			path = "redirect:/board/reviewDelete.aws?bidx=" + bidx;
	...
	@RequestMapping(value = "levelupDeleteAction.aws", method = RequestMethod.POST) // 리뷰 삭제기능
	public String levelupDeleteAction(@RequestParam("bidx") int bidx, @RequestParam("pwd") String pwd,
			RedirectAttributes rttr, HttpSession session) {
		...
		if (value == 1) {
			path = "redirect:/board/levelupList.aws"; // 성공하면 리스트페이지로 보내주고
			System.out.println("path : " + path);
		} else {
			rttr.addFlashAttribute("msg", "비밀번호가 틀렸습니다");
			path = "redirect:/board/levelupDelete.aws?bidx=" + bidx;
			System.out.println("path : " + path);// 비밀번호가 틀려서 실패하면 bidx 를 가지고 삭제페이지로 보내기
		}
	...
	@RequestMapping(value = "askDeleteAction.aws", method = RequestMethod.POST) // 리뷰 삭제기능
	public String askDeleteAction(@RequestParam("bidx") int bidx, @RequestParam("pwd") String pwd,
			RedirectAttributes rttr, HttpSession session) {
		...
		if (value == 1) {
			path = "redirect:/board/askList.aws"; // 성공하면 리스트페이지로 보내주고
			System.out.println("path : " + path);
		} else {
			rttr.addFlashAttribute("msg", "비밀번호가 틀렸습니다");
	...
	@RequestMapping(value = "reservationDeleteAction.aws", method = RequestMethod.POST) // 리뷰 삭제기능
	public String reservationDeleteAction(@RequestParam("bidx") int bidx, @RequestParam("pwd") String pwd,
			RedirectAttributes rttr, HttpSession session) {
		...
		if (value == 1) {
			path = "redirect:/board/reservationList.aws"; // 성공하면 리스트페이지로 보내주고
			System.out.println("path : " + path);
		} else {
			rttr.addFlashAttribute("msg", "비밀번호가 틀렸습니다");
			path = "redirect:/board/reservationDelete.aws?bidx=" + bidx;
			System.out.println("path : " + path);// 비밀번호가 틀려서 실패하면 bidx 를 가지고 삭제페이지로 보내기
		}
	...
	@RequestMapping(value = "boardReply.aws") // 답변하기는 문의ㄱ
	...
	public String boardReplyAction(BoardVo bv, // 제목, 내용, 작성자, 비밀번호 bv에 담아오기
		...
		String path = "";
		if (maxBidx != 0) {
			path = "redirect:/board/askContents.aws?bidx=" + maxBidx; // 결과값이 0이 아닌경우 > 성공한것이므로 글내용으로 보내기 + 여기서 사용하려고 위에
																		// 리다이렉트 객체 생성한 것
		} else {
	...
	public JSONObject boardRecom(@RequestParam("bidx") int bidx) { // 리턴값 JSONObject 으로 설정 > @ResponseBody 해서 객체를 받는다고
																	// 설정
	...
	public ResponseEntity<byte[]> displayFile(@RequestParam("fileName") String fileName, // 파일이름 넘기는 파라미터
		...
		try {
			String formatName = fileName.substring(fileName.lastIndexOf(".") + 1);// 자리수 계산한다음에 +1해주고 확장자 확인하는
																					// ..substring(마지막. 뒤에있는 문자를 자른다 >
																					// 확장자 확인)
