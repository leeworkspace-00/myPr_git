...
<%
 ArrayList<BoardVo> llist = (ArrayList<BoardVo>)request.getAttribute("llist"); // 형변환 때문에 경고라인 뜬거 걱정 말기
...
<table class="listTable">
		...
		<tr>
			...
			<td class="title">
			<%=lvlStr %>
			<a href="<%=request.getContextPath() %>/board/levelupContents.aws?bidx=<%=bv.getBidx() %>"><%=bv.getSubject() %></a></td>
			<td><%=bv.getWriter()%></td>
	...
	<div class="btnBox">
		<a class="btn aBtn" href="<%=request.getContextPath() %>/board/levelupWrite.aws">지원</a>
	</div>
